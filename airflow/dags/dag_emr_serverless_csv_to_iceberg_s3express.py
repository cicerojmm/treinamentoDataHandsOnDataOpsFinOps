...
S3_EXPRESS_WAREHOUSE = "{{ var.value.s3_express_warehouse }}"

@dag(
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    tags=['emr-serverless', 'etl', 'iceberg', 's3-express']
)
def dag_emr_serverless_csv_to_iceberg_s3express():
    ...
    emr_job = EmrServerlessStartJobOperator(
        ...
        job_driver={
            "sparkSubmit": {
                "entryPoint": f"s3://{S3_BUCKET_CONFIG}/scripts/emr/emr_csv_to_iceberg_s3express.py",
                "entryPointArguments": [
                ...
                "sparkSubmitParameters": (
                    ...
                    f"--conf spark.sql.catalog.glue_catalog.warehouse={S3_EXPRESS_WAREHOUSE} "
